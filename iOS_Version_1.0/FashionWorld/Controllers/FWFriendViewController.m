...
@interface FWFriendViewController () <FWContentCellDelegate>
...
@implementation FWFriendViewController
...
- (void)fetchDataWithRefresh:(BOOL)isRefresh
{
    ...
    NSString* publishType = @"1,2";
    _strUrl = [NSString stringWithFormat:@"%@%@%@/%ld/%@/%ld", DDXAPI_HTTP_PREFIX_Friend, DDXAPI_SHOPFRIEND_SR, DDXAPI_SHOPFRIEND_SR_CF_LIST, [DDXUserinfo getUserId], publishType, (long)_pageIndex];
...
//                NSLog(@"json == %@", json);
                ...
                if (isSuccess) {
                    NSArray *items = contentDic[@"rows"];
                    models = [NSArray osc_modelArrayWithClass:[FWContentItem class] json:items].mutableCopy;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.contents.count) {
        ...
        if (commentItem.publishType == CircleFriendsPublicTypeText) {
            ...
            return cell;
        } else if (commentItem.publishType == CircleFriendsPublicTypeImage) {
            ...
            return cell;
        } else if (commentItem.publishType == CircleFriendsPublicTypeVedio) {
