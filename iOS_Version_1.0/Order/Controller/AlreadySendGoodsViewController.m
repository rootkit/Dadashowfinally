//
//  AlreadySendGoodsViewController.m
//  iOS_Version_1.0
//
...
#import "OrderDrawbackViewController.h"// 退款
#import "OrderLogisticsDetailViewController.h"//物流
#import "GuessLikeCell.h"

#define  headheight 100*(kScreen_Height/667)
#define addressheight 82*(kScreen_Height/667)
#define goodsviewheght 265*(kScreen_Height/667)
#define ordercodeheight 153*(kScreen_Height/667)
...
@interface AlreadySendGoodsViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
@property (nonatomic, strong) UICollectionView *collectionView;
@property(nonatomic,strong)UIView* collectheadview;
@property (nonatomic, assign) CGFloat viewHeight;
@end
@implementation AlreadySendGoodsViewController
...
-(void)setordergoodsview{
    ...
    reasonimageview.image=[UIImage imageNamed:@"ico_car"];
    [headview addSubview:reasonimageview];
    [reasonimageview mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(@(10*(kScreen_Height/667)));
        make.width.equalTo(@(100*(kScreen_Height/667)));
    ...
    [logisticsview mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(82*(kScreen_Height/667)));
    }];
    _viewHeight=_viewHeight+82*(kScreen_Height/667);
    ...
    definitelabel.text = @"[深圳市]在广东深圳公司南山区域粤海街道科兴科学园进行扫描签收,快件已被签收,感谢使用顺丰快递,期待再次为您服务";
    ...
    _viewHeight=_viewHeight+1*(kScreen_Height/667)+addressheight;
    ...
    UIButton * reimbursebtn=[[UIButton alloc]init];
    reimbursebtn.titleLabel.font=[UIFont systemFontOfSize:13*(kScreen_Height/667)];
    [reimbursebtn setTitle:@"退款"forState:UIControlStateNormal];
    [reimbursebtn setTitleColor:[UIColor colorWithHex:0x353535] forState:UIControlStateNormal];
    reimbursebtn.layer.cornerRadius = 4*(kScreen_Height/667);
    reimbursebtn.layer.borderWidth = 0.4;
    reimbursebtn.layer.borderColor = [UIColor colorWithHex:0x333333].CGColor;
    reimbursebtn.layer.masksToBounds=YES;
    [reimbursebtn addTarget:self action:@selector(clickreimburse) forControlEvents:UIControlEventTouchUpInside];
    [backview addSubview:reimbursebtn];
    [reimbursebtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(@(-14*(kScreen_Height/667)));
        make.right.equalTo(@(-12*(kScreen_Height/667)));
        make.width.equalTo(@(40*(kScreen_Height/667)));
    ...
    [travellinglabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(shopgoodsimage.mas_bottom).equalTo(@(58*(kScreen_Height/667)));
        make.left.equalTo(shopnameimageview.mas_left);
    }];
    ...
    UILabel* paytimes = [UILabel new];
    paytimes.text = @"付款时间:2017-05-05 19:30:00";
    paytimes.font = [UIFont systemFontOfSize:11*(kScreen_Height/667)];
    paytimes.textColor = [UIColor colorWithHex:0x949393];
    [extraview addSubview:paytimes];
    [paytimes mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [bottleline mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(createtimes.mas_bottom).equalTo(@(32*(kScreen_Height/667)));
        make.left.equalTo(@0);
        make.right.equalTo(@-0);
        make.height.equalTo(@1);
    }];
    ...
    surebtn.titleLabel.font=[UIFont systemFontOfSize:13*(kScreen_Height/667)];
    [surebtn setTitle:@"确认收货"forState:UIControlStateNormal];
    [surebtn setTitleColor:[UIColor colorWithHex:0xfc5c98] forState:UIControlStateNormal];
    surebtn.layer.cornerRadius = 4*(kScreen_Height/667);
    surebtn.layer.borderWidth = 0.4;
    surebtn.layer.borderColor = [UIColor colorWithHex:0xfc5c98].CGColor;
    surebtn.layer.masksToBounds=YES;
    [surebtn addTarget:self action:@selector(clicksure) forControlEvents:UIControlEventTouchUpInside];
    [extraview addSubview:surebtn];
    [surebtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(@(-11*(kScreen_Height/667)));
        make.right.equalTo(@(-12*(kScreen_Height/667)));
    ...
    checkbtn.titleLabel.font=[UIFont systemFontOfSize:13*(kScreen_Height/667)];
    [checkbtn setTitle:@"查看物流"forState:UIControlStateNormal];
    [checkbtn setTitleColor:[UIColor colorWithHex:0x353535] forState:UIControlStateNormal];
    checkbtn.layer.cornerRadius = 4*(kScreen_Height/667);
    checkbtn.layer.borderWidth = 0.4;
    checkbtn.layer.borderColor = [UIColor colorWithHex:0x333333].CGColor;
    ...
    [extraview addSubview:checkbtn];
    [checkbtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(@(-11*(kScreen_Height/667)));
        make.right.equalTo(surebtn.mas_left).equalTo(@(-10*(kScreen_Height/667)));
        make.width.equalTo(@(80*(kScreen_Height/667)));
        make.height.equalTo(@(26*(kScreen_Height/667)));
    }];
    
    UIButton * delaybtn=[[UIButton alloc]init];
    delaybtn.titleLabel.font=[UIFont systemFontOfSize:13*(kScreen_Height/667)];
    [delaybtn setTitle:@"延长收货"forState:UIControlStateNormal];
    [delaybtn setTitleColor:[UIColor colorWithHex:0x353535] forState:UIControlStateNormal];
    delaybtn.layer.cornerRadius = 4*(kScreen_Height/667);
    delaybtn.layer.borderWidth = 0.4;
    delaybtn.layer.borderColor = [UIColor colorWithHex:0x333333].CGColor;
    delaybtn.layer.masksToBounds=YES;
    [delaybtn addTarget:self action:@selector(clickdelay) forControlEvents:UIControlEventTouchUpInside];
    [extraview addSubview:delaybtn];
    [delaybtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(@(-11*(kScreen_Height/667)));
        make.right.equalTo(checkbtn.mas_left).equalTo(@(-10*(kScreen_Height/667)));
        make.width.equalTo(@(80*(kScreen_Height/667)));
        make.height.equalTo(@(26*(kScreen_Height/667)));
    }];
...
-(void)clickreimburse{
    NSLog(@"还未申请退款显示“申请退款”，跳至“申请退款”页面；已申请退款显示“退款中”，跳至“退款详情”页面");  //
