...
@interface TextRoomViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong)UITableView* textroomtableview;
...
@implementation TextRoomViewController
...
- (UITableView *)textroomtableview
{
    if (!_textroomtableview) {
        ...
        _textroomtableview = tableView;
    }
    return _textroomtableview;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (index==1) {
        NSLog(@"奇数");
        AllsecondTableViewCell *cell = [AllsecondTableViewCell returnResueCellFormTableView:self.textroomtableview indexPath:indexPath identifier:AllsecondTableViewCellIdentifier];
        return cell;
        
    }
    else{
        NSLog(@"偶数");
        AllgoodsTableViewCell *cell = [AllgoodsTableViewCell returnResueCellFormTableView:self.textroomtableview indexPath:indexPath identifier:AllgoodsTableViewCellIdentifier];
        return cell;
...
-(void)changeablescorll{
    _textroomtableview.scrollEnabled=YES;
}
-(void)allscorllable{
    _textroomtableview.scrollEnabled=NO;
}
